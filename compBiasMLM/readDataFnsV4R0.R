...
#rhs <- function(x) {
#    as.formula(paste0("~",strsplit(as.character(x),"~")[[1]][2]))
#}
#lhs <- function(x) {
#    strsplit(as.character(x),"~")[[1]][1]
#}
#modified funct to be less IEA specific...

makeCdat <- function(fmla,data,groupFmla,cntrTreat=T, stdz=T,tol=1e-2) {

   #remove NAs & process the formula:
   groupName <- as.character(rhs(groupFmla))
   respName <- as.character(lhs(fmla))
   ...
   if (stdz) cdat <- as.data.frame(sapply(cdat,scaleAll)) #still use scaleAll as there may be lurking factors (not a problem if they can be treated as numeric values - e.g., sex 0/1).
   ...
   #might need to trap condition of no variation within group anywhere, e.g.
   #at this point, the treatment is first and the response is last in the df.  Exploit this structure.
   ...
   if (!cntrTreat) {    #put back mean if response variable, and flag set:
     df.X[,treatName] <- treatVar
   } 
   datNew <- cbind(df.X,df.Worig)
   ## Prepare the formulas for the model fit calls.
   #indiv-level preds
   #need to use `` due to factor vars and logged vars...  should work, though clumsy
   ...
   fmlaY <- as.formula(paste0("~",respName))
    ...
    ####################################
   return(list(cdat=datNew,fmlaY=fmlaY,fmlaZ=fmlaZ,fmlaZ.mn=fmlaZ.mn, fmlaX=fmlaX,fmlaW=fmlaW,group=groupFmla))
}
