...
# some of the next 3 functions return lists with one object.  This is due to prior need for returning
# multiple objs and the desire not to recode everything that depends on them.  Returned objs could be
# simplified if dependencies handled.
lmExtract <- function(lmObject, treatName) {
    # extract tau (coef) corresponding to treatment Z & s.e.(tau) based only on 'treatName' entry
...
lmeExtract <- function(lmeObject, treatName, treatGpName) {
    # works on lmer obj extract fixef of treatment (two forms - 2nd one is group mean) fixef:
...
lmeExtract.varcomp <- function(lmeObject) {
    # extract variances of random intercept model & ln(vcv) of those: currently works on lmer only objects
    # variances:
...
geoMeanGroupSize <- function(groupID) {
    prod(table(groupID))^(1/length(unique(groupID)))
}
...
prepDat <- function(outcome, treatment, level1.pred, level2.pred, group, data) {
    ...
    if (!is.na(match(z.mn.name, dataNames))) 
        z.mn <- data[, z.mn.name] else z.mn <- 0  #should work for the special case of not using runModels to build dat0
...
runModels <- function(outcome = ~Y, treatment = ~Z, level1.pred = ~X1 + X2, level2.pred = ~W, group = ~id, 
    data) {
    
    # all formula components are given using formula notation group identifier data: the dataframe already
    # group mean centered ...
    ...
    # even though it's not req'd to run stan after runModels, this is useful for stan call and a few variance
    # calcs, so we do it here and pass the results forward.
    ...
    list(ols0.y = ols0.y, ols1.y = ols1.y, mlm.ucm.y = mlm.ucm.y, mlm0.y = mlm0.y, mlm1.y = mlm1.y, mlm0.z = mlm0.z, 
        mlm1.z = mlm1.z, outcome = outcome, treatment = treatment, level2.pred = level2.pred, varX = varX, 
        varW = varW, varZ = varZ, data = data, preppedData = pDat)
}
