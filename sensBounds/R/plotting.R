...
prePlotParams <- function(mdlFit, nGridPoints = 201, tau.max = 1, gpSize = Inf) {
    ################################ Bounds calcs params needed for plots cond numb nGridPoints: number of points to build the confounder
    ################################ line.  tau.max: abs(tau) used in the bounds calculation (plausible value supplied by user) gpSize: for
    ################################ balanced designs, the number of subjects/group.  For others, try geom. mean or Inf.
